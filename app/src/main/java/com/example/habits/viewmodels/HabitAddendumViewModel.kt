...
class HabitAddendumViewModel(
    ...
    private val previousType: HabitType,
    ...
    fun addHabit() {
        _habit.value?.let { viewModelScope.launch { repository.insert(it) } }
    }

    fun changeHabit() {
        _habit.value?.let { viewModelScope.launch { repository.update(it) } }
    }
